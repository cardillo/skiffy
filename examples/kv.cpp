...
static void print_kv(raftpp::server_id id,
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        std::cerr << "usage: kv <id> <port> <host>"
    ...
    node.on_apply([id, &kv, &node](const log_entry& e) {
    ...
    std::thread submit_th([&node, id, quorum] {
        ...
        std::uniform_int_distribution<int> ms_dist(500, 3000);

        while (node.running()) {
            // wait until cluster has quorum
            if (node.peer_count() + 1 < quorum) {
