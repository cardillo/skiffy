...
struct message {
    ...
    void msgpack_unpack(msgpack::object const& o) {
        ...
        type   = static_cast<msg_type>(
                     a.ptr[0].as<uint8_t>());
        term   = a.ptr[1].as<term_t>();
        from   = a.ptr[2].as<server_id>();
        to     = a.ptr[3].as<server_id>();
        if (a.ptr[4].type  == nil) last_log_term  = {};
        else last_log_term  = a.ptr[4].as<term_t>();
        if (a.ptr[5].type  == nil) last_log_index = {};
        else last_log_index = a.ptr[5].as<index_t>();
        if (a.ptr[6].type  == nil) vote_granted   = {};
        else vote_granted   = a.ptr[6].as<bool>();
        if (a.ptr[7].type  == nil) prev_log_index = {};
        else prev_log_index = a.ptr[7].as<index_t>();
        if (a.ptr[8].type  == nil) prev_log_term  = {};
        else prev_log_term  = a.ptr[8].as<term_t>();
        if (a.ptr[9].type  == nil) { entries = {}; }
        else {
            std::vector<log_entry> v;
            a.ptr[9].convert(v);
            entries = std::move(v);
        }
        if (a.ptr[10].type == nil) commit_index   = {};
        else commit_index   = a.ptr[10].as<index_t>();
        if (a.ptr[11].type == nil) success        = {};
        else success        = a.ptr[11].as<bool>();
        if (a.ptr[12].type == nil) match_index    = {};
        else match_index    = a.ptr[12].as<index_t>();
        if (a.ptr[13].type == nil) snapshot_index = {};
        else snapshot_index = a.ptr[13].as<index_t>();
        if (a.ptr[14].type == nil) snapshot_term  = {};
        else snapshot_term  = a.ptr[14].as<term_t>();
        if (a.ptr[15].type == nil) snapshot_data  = {};
        else snapshot_data  =
            a.ptr[15].as<std::string>();
        if (a.ptr[16].type == nil) client_data    = {};
        else client_data    =
            a.ptr[16].as<std::string>();
    }
};
...
struct memory_log_store {
    ...
    const log_entry& operator[](size_t i) const {
        return entries_[i];
    }
...
public:
    ...
    server(server_id self,
        ...
        : log_store_own_{}
        , state_machine_own_{}
        , log_store_(log_store_own_)
        , state_machine_(state_machine_own_)
    ...
    void compact() {
        if (commit_index_ <= snapshot_index_) return;
        ...
        size_t keep_from =
            new_snap - snapshot_index_;
        for (size_t i = keep_from;
             i < log_store_.size(); ++i) {
    ...
    const std::vector<log_entry>& log() const {
        return log_store_.entries();
    }
    const std::set<server_id>& votes_responded()
        const
    {
        return votes_responded_;
    }
    const std::set<server_id>& votes_granted()
        const
    {
    ...
    const std::optional<std::set<server_id>>&
        joint_config() const
    {
    ...
    term_t last_term() const {
        if (log_store_.empty()) return snapshot_term_;
        ...
        const std::set<server_id>& s) const
    {
        ...
        size_t new_sz =
            joint_config_->size() + 1;
        return count_in(peers_) * 2 > old_sz
            && count_in(*joint_config_) * 2
               > new_sz;
    }
...
private:
    ...
    size_t log_offset(index_t idx) const {
        return idx - snapshot_index_ - 1;
    }
        ...
        const message& m)
    {
        ...
        term_t m_last_log_term =
            m.last_log_term.value_or(0);
        index_t m_last_log_index =
            m.last_log_index.value_or(0);
    ...
    void handle_request_vote_response(
        const message& m)
    {
        if (m.term != current_term_) return;
        ...
        const message& m)
    {
        server_id j       = m.from;
        index_t prev_idx  =
            m.prev_log_index.value_or(0);
        term_t  prev_term =
            m.prev_log_term.value_or(0);
        const auto& entries =
            m.entries.value_or(
                std::vector<log_entry>{});
        index_t m_commit =
            m.commit_index.value_or(0);

        bool log_ok =
            prev_idx == 0
            || prev_idx <= snapshot_index_
            || (prev_idx > snapshot_index_
        ...
        for (size_t i = 0;
            ...
            if (idx <= snapshot_index_) continue;
            size_t si = log_offset(idx);
            if (si < log_store_.size()) {
                if (log_store_[si].term
                    != entries[i].term) {
    ...
    void handle_install_snapshot_request(
        const message& m)
    {
        if (m.term < current_term_) return;
        index_t si =
            m.snapshot_index.value_or(0);
        term_t  st =
            m.snapshot_term.value_or(0);
        if (snapshot_index_ >= si) return;

        state_machine_.install(
            m.snapshot_data.value_or(""));
        snapshot_index_ = si;
        snapshot_term_  = st;
        ...
        message resp;
        resp.type           =
            msg_type::install_snapshot_resp;
        resp.term           = current_term_;
    ...
    void revert_config_if_needed(
        size_t from_offset)
    {
        for (size_t i = from_offset;
             i < log_store_.size(); ++i) {
            index_t idx =
                snapshot_index_
                + 1
                + static_cast<index_t>(i);
            if (idx <= last_applied_) continue;
            if (log_store_[i].type ==
    ...
    void apply_committed() {
        for (index_t idx = last_applied_ + 1;
             idx <= commit_index_; ++idx) {
            if (idx <= snapshot_index_) continue;
            auto& e =
                log_store_[log_offset(idx)];
        ...
        if (compact_threshold_ > 0 &&
            log_store_.size() >= compact_threshold_)
            compact();
    }
...
struct sim_transport {
    ...
    const std::deque<message>& pending() const {
        return queue_;
    }
    ...
    void deliver(Fn&& fn) {
        ...
        std::uniform_real_distribution<double>
            dist{0.0, 1.0};
        for (auto& msg : tmp) {
            if (dist(rng_) < drop_rate) continue;
            fn(msg);
            if (dist(rng_) < dup_rate) fn(msg);
        }
    }
...
public:
    ...
    void add_peer(server_id id, endpoint_t ep) {
        peers_[id] = ep;
        logger()->debug(
            "transport {} add_peer {} at {}:{}",
            self_, id,
...
private:
            ...
            msgpack::unpacker> unpacker)
    {
        ...
        sock->async_read_some(
            asio::buffer(
                unpacker->buffer(), read_buf_size),
            [this, sock, unpacker](
                asio::error_code ec, size_t n)
            {
                if (ec) {
                    logger()->debug(
                        "transport {} read"
...
struct member_info {
    ...
    asio::ip::tcp::endpoint mem_endpoint()
        const
    {
        return {asio::ip::make_address(host),
                mem_port};
...
struct mem_message {
    ...
    void msgpack_unpack(msgpack::object const& o) {
        auto& a   = o.via.array;
        auto  nil = msgpack::type::NIL;
        type = static_cast<mem_msg_type>(
                   a.ptr[0].as<uint8_t>());
        if (a.ptr[1].type == nil) joiner_id = {};
        else joiner_id = a.ptr[1].as<server_id>();
        if (a.ptr[2].type == nil) joiner_host = {};
        else joiner_host =
                 a.ptr[2].as<std::string>();
        if (a.ptr[3].type == nil)
            joiner_raft_port = {};
        else joiner_raft_port =
                 a.ptr[3].as<uint16_t>();
        if (a.ptr[4].type == nil)
            joiner_mem_port = {};
        else joiner_mem_port =
                 a.ptr[4].as<uint16_t>();
        if (a.ptr[5].type == nil) { members = {}; }
        else {
...
public:
    using endpoint_t = asio::ip::tcp::endpoint;
    using on_peer_added_t =
        std::function<
            void(server_id, endpoint_t)>;
    using on_peer_removed_t =
        std::function<void(server_id)>;
    ...
    void join(endpoint_t bootstrap_mem_ep) {
        logger()->info(
            "server {} joining via {}:{}",
            self_,
            bootstrap_mem_ep.address()
                .to_string(),
        ...
        if (resp.members) {
            ...
            for (auto& mi : members_) {
                ...
                mem_message ann;
                ann.type            =
                    mem_msg_type::announce;
                ann.joiner_id        = self_;
                ann.joiner_host      =
                    self_host_;
                ann.joiner_raft_port =
                    self_raft_port_;
                ann.joiner_mem_port  =
                    self_mem_port_;
    ...
    // Must be called before run() — not thread-safe.
    void set_on_peer_removed(
        on_peer_removed_t cb)
    {
        on_peer_removed_ = std::move(cb);
    }

    const std::vector<member_info>&
        members() const
    {
        return members_;
    }
...
private:
        ...
        const mem_message& msg)
    {
        if (msg.type ==
            ...
            mi.host =
                msg.joiner_host.value_or("");
            mi.raft_port =
                msg.joiner_raft_port
                    .value_or(0);
            mi.mem_port =
                msg.joiner_mem_port
                    .value_or(0);
            ...
            bool dup = false;
            for (auto& x : members_)
                if (x.id == mi.id) dup = true;
            if (!dup) members_.push_back(mi);
...
private:
    ...
    std::uniform_int_distribution<int>
        election_dist_;
    std::function<void(const log_entry&)>
        on_apply_;
