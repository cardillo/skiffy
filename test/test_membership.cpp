...
TEST_CASE("mem_message announce encode/decode") {
    ...
    CHECK(msg2.joiner_host.value_or("")
          == "192.168.1.1");
...
TEST_CASE("leader crash before config_joint"
    ...
    v.vote_granted = true;
    v.to           = 1;
    v.from = 2; s1.receive(v);
    v.from = 3; s1.receive(v);
    s1.become_leader();
    t.clear();
...
TEST_CASE("membership_manager: join flow") {
    ...
    t1.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19101));

    membership_manager mgr1(1, io1, t1);
    mgr1.set_self_info("127.0.0.1", 19101, 19201);
    mgr1.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19201));
    ...
    mgr1.set_on_peer_added(
        [&](server_id id, tcp::endpoint) {
            if (!promise_set) {
                promise_set = true;
    ...
    mgr2.join(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19201));

    // wait for bootstrap to process the announce
    auto status = peer_future.wait_for(
...
TEST_CASE("on_peer_added fires on config"
    ...
    s2.set_on_peer_added(
        [&](server_id pid) { added_pid = pid; });
...
TEST_CASE("on_peer_removed fires on config"
    ...
    s1.set_on_peer_removed(
        [&](server_id pid) { removed_pid = pid; });
    ...
    // fake acks from s2 and s3 for index 1
    {
        ...
        resp.match_index = 1;
        resp.to = 1;
        resp.from = 2; s1.receive(resp);
        resp.from = 3; s1.receive(resp);
    }
