...
TEST_CASE("remove_peer cleans up server state") {
    ...
    bool threw2 = false;
    try { s.next_index_for(2); }
    catch (...) { threw2 = true; }
    CHECK(threw2);

    bool threw3 = false;
    try { s.next_index_for(3); }
    catch (...) { threw3 = true; }
    CHECK(!threw3);
}
...
TEST_CASE(
    "membership_manager handles leave_req")
{
    ...
    tA.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19301));

    membership_manager mgrA(1, ioA, tA);
    mgrA.set_self_info(
        "127.0.0.1", 19301, 19401);
    mgrA.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19401));
...
TEST_CASE(
    "membership_manager handles remove")
{
    using namespace asio;
    using tcp = ip::tcp;
    ...
    asio_transport tA(1, ioA);
    tA.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19305));

    membership_manager mgrA(1, ioA, tA);
    mgrA.set_self_info(
        "127.0.0.1", 19305, 19405);
    mgrA.listen(tcp::endpoint(
        ip::make_address("127.0.0.1"), 19405));
